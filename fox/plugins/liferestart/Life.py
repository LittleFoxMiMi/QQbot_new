...
class Life:
                ...
                for t2 in self.talent.talents:
                    if t2.isExclusiveWith(t):
                        raise HandlerException(
                            f'talent chosen conflict with {t2}')
                self.talent.addTalent(t)
    ...
    def setproperty(self, protery_list):
        self.property.CHR += int(protery_list["CHR"])
        self.property.INT += int(protery_list["INT"])
        self.property.STR += int(protery_list["STR"])
        self.property.MNY += int(protery_list["MNY"])
