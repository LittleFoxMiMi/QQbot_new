...
from ..auto_mys.exception_handler import exception_handler

json_path = os.path.abspath('./fox/data/paimon/')+"/cloud_genshin.json"
...
cloud_ys = on_command('云原神', priority=1, block=True)
rm_cloud_ys = on_command('云原神解绑', priority=1, block=True)
...
for user_id, data in load_json(json_path).items():
    scheduler.add_job(
        func=auto_sign_cgn,
        trigger='cron',
        hour=6,
