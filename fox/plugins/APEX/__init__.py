...
apexhelp = on_command("apex", priority=5, block=True)
...
async def _(bot: Bot, event: MessageEvent, state: T_State):
    help_info = "a地图:当前轮换地图\na制造:今日制造轮换\na猎杀:查询猎杀底分\n\
    \a查询:使用“a查询 [橘子id]”来查询战绩和段位\na绑定:将橘子id与qq绑定以便快速查询"
    await apexhelp.finish(help_info)
...
async def _(bot: Bot, event: Event, text: Message = CommandArg()):
    ...
    if len(args) >= 1:
        response = requests.get(
            f"https://api.mozambiquehe.re/bridge?auth={Tool_Token}&player={args[0]}&platform=PC")
        uid = args[0]
        if response.status_code != 200:
